...
phase: backlog build — A2 cross-link /games ↔ /arcade on botsite: the two overlapping game surfaces were only reachable one way (`/games` already linked to `/arcade`, but `/arcade` was a dead end back to `/games`); A2 adds the missing reverse cross-link on `botsite/templates/arcade.html`, mirroring the existing games→arcade strip, so the two surfaces are mutually reachable. Branch `claude/arcade-games-crosslink`; a NEXT-TASKS slice of the standing overnight ORDER 032. Template/nav only — no product logic, no serialized JSON/env, no route change.
health: green — origin/main advanced: C14 self-cleaning owner queue (#386), the orphan-claim cleanup (#387), B1 arcade live/blocked counts on dashboard /status (#388), C1 honest counts on /work /history /console (#389), A4 arcade JSON schema CI guard (#390), R6 review bake-generator unit tests (#391), and C15 durable ask ids on the owner-action queue (#392) all landed. Last full four-suite run 1806 (2026-07-18, this session — 1804 + 2 new cross-link tests).
last-shipped: #392 — origin/main head 85539e0.
orders: acked=001-032 done=001-019,023-031 — unchanged. A2 ships as a backlog slice of the standing overnight ORDER 032, not an order completion.
...
blockers: none
landing: pushed-unmerged claude/arcade-games-crosslink — PR #393 READY (A2 /arcade↔/games cross-link); merges via server-side auto-merge-enabler on green (agents cannot self-merge, classifier since 2026-07-15). Open PRs: this one.
deployed: 85539e0 · four Railway services live (control-plane/botsite/dashboard/review). A2 touches botsite only — `botsite/templates/arcade.html` (the reverse cross-link to /games) + `botsite/tests/test_arcade.py` (both-direction assertions). No env, no workflow, no serialized JSON, no route change; all links are GET, so the CSRF/same-origin floor is untouched.
...
notes: WIND-DOWN standing — the Claude Code Projects EAP goes READ-ONLY 2026-07-21; the autonomous apparatus is being retired and the Project recreated (inventory: docs/NEXT-TASKS.md → "Wind-down / retirement"). A2 is a contained, reversible botsite nav slice, independent of the retirement set.
...
## NEXT-2-TASKS baton
1. B6 — config-drift flags on the dashboard /env surface.
2. R1 — surface /questions in the review NAV (NAV-add only; the empty-state is deliberate).
