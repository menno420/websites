...
done-when: dashboard /admin/login + /admin/auth/callback live; the "OAuth never configured" stub replaced by the real flow; the owner actor is real when signed in; state-changing /admin actions gated fail-closed behind the owner session; tests green; landed and live-verified.

**Provenance:** coordinator directive (cross-session, continuing the owner's live 2026-07-19 ~09:30Z direction "Can't we just use te discord login for everything?"), approving the forward idea captured in the ORDER 037 session card (dashboard's admin was a Discord-OAuth display stub — ANONYMOUS_ACTOR placeholder, admin_login.html "not configured, never will be").

**Decision:** Port app/discord_auth.py / botsite/discord_auth.py into dashboard/discord_auth.py (duplication per service, never a cross-service import). Mount the router at /admin/login + /admin/auth/callback (dashboard's admin namespace; the existing "not configured" /admin/login stub route + admin_login.html copy are replaced), success redirects to /admin. Discord-ONLY: dashboard has never had a SITE_PASSWORD gate (ASK-0009: its SITE_PASSWORD is unused/deletable), so no HTTP-Basic fallback is added. The dry-run /admin control actions (POST /admin/actions/preview + /admin/actions/confirm) are gated fail-closed behind the owner session (503 when OAuth unconfigured); the /admin read views stay public — mirroring botsite's shape (public surface open, owner surface gated). ANONYMOUS_ACTOR is used only when no valid session; a valid Discord owner session makes the actor the real owner id. Same four env var names + optional DISCORD_REDIRECT_URI, reused from the same SuperBot Discord app; owner unlock = add the dashboard redirect URI + paste the same four vars on the dashboard service.

## ORDER 039 · 2026-07-20T13:22:05Z · status: new
priority: P1
do: Your substrate-kit v1.20.1 upgrade PR #452 is red on its substrate-gate: the new gate flags pre-existing false-wall findings in this repo's docs (see the PR body's finding list). Fix on the PR branch with plain additive commits: rephrase each flagged standing-limitation line as a dated past-tense fact or add a dated supersession note (never delete history; owner-verbatim lines go in the checker's allowlist if supported). Reference implementation: fleet-manager PR #390 (commit d0e16e2). Gate green means the armed auto-merge lands the upgrade.
...
done-when: PR #452's substrate-gate is green and the armed auto-merge has landed the v1.20.1 upgrade on main.
