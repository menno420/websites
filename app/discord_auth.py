"""Discord OAuth owner login for the control-plane (ASK-0001 / ORDER 035).

The NON-gated half of the ASK-0001 decision: an authorization-code Discord
login flow that mints a signed owner session on the control-plane (`app/`)
owner surface. It REUSES the existing fleet-side SuperBot Discord app — the
owner adds a redirect URI + copies the client id/secret onto this service's
Railway env (ASK-0002); until then every gated surface stays locked and the
login page names exactly which owner action opens it.
...
* `oauth_configured()` is True ONLY when all four of DISCORD_CLIENT_ID,
  ...
  200) naming ASK-0002/ASK-0003; the callback 503s.
* The session cookie is an HMAC-SHA256 signature (stdlib only — `hmac`,
  `hashlib`, `secrets`, `base64`) over ``discord_id|issued_at|expiry`` keyed
  on OWNER_SESSION_SECRET; `verify_session` returns the id only on a
...
The two network calls are factored into `exchange_code`/`fetch_user` seams so
tests monkeypatch them without live Discord. This module builds NO armed
bot-control write path — that stays a separate armed Railway service
(ASK-0003), owner-gated.

Wiring: `owner_session_id(request)` is the reader `app/owner.py`'s
...
The router here is included by `app/main.py` and is deliberately NOT under the
owner gate (it is the door, not a room behind it).
"""
...
# Mirror app/main.py + app/owner.py: this module renders base.html through its
# OWN Jinja env, so without these globals the header nav would be empty.
...
# newly-pasted env is picked up without a restart, mirroring config.deployed_sha.
...
def owner_session_id(request: Request) -> str | None:
    ...
    or None. The reader `app/owner.py`'s `require_owner` consults first."""
...
# --------------------------------------------------------------------------- #
# routes — included by app/main.py, NOT under the owner gate.
# --------------------------------------------------------------------------- #
def _login_context(request: Request, configured: bool) -> dict:
    """Context for owner_login.html. Deliberately carries NO nav highlight key —
    this is an auth-flow endpoint, not a nav page (keeps the nav-manifest guard
    unaffected)."""
    return {
        "request": request,
...
async def owner_login(request: Request):
    ...
    naming the opening owner actions (ASK-0002 redirect URI + env, ASK-0003).
    Configured → set the signed state cookie and 302 to Discord's authorize
    endpoint (scope=identify)."""
    ...
    if not oauth_configured():
        raise HTTPException(
            ...
            detail=(
                ...
                f"{ENV_SESSION_SECRET} on this service (ASK-0002)."
            ),
        )
...
async def owner_logout(request: Request):
    ...
    is rejected 403), mirroring the /owner state-change floor."""
