...
A near-verbatim port of the control-plane's ``app/discord_auth.py`` into the
public bot site, so ONE Discord login gates botsite's owner surfaces — the
tester-program owner queue (``/testing/owner``) and the submissions moderation
feed (``/submit/queue.json``). botsite never imports ``app/`` (services never
share a process), so this is a deliberate idiomatic DUPLICATION, not an import.

It REUSES the existing fleet-side SuperBot Discord app — the owner adds a
botsite redirect URI + copies the client id/secret onto botsite's OWN Railway
service (the botsite equivalent of ASK-0002); until then every gated surface
stays locked and the login page names exactly which owner action opens it.
``SITE_PASSWORD`` remains an OPTIONAL HTTP-Basic fallback.
...
* ``oauth_configured()`` is True ONLY when all four of DISCORD_CLIENT_ID,
  ...
  Env-unset → ``/owner/login`` renders an honest "not configured" page (HTTP
  200) naming the four vars + the redirect URI; the callback 503s.
...
Wiring: ``owner_session_id(request)`` is the reader ``botsite/testing.py``'s
``require_owner`` consults FIRST — a valid owner session authorizes the whole
gated area, the existing HTTP-Basic SITE_PASSWORD path stays the fallback.
The router here is included by ``botsite/app.py`` and is deliberately NOT under
the owner gate (it is the door, not a room behind it).
"""
...
def owner_session_id(request: Request) -> str | None:
    ...
    or None. The reader ``botsite/testing.py``'s ``require_owner`` consults
    first."""
...
def redirect_uri_for(request: Request) -> str:
    ...
    derived from the request's own base URL + /owner/auth/callback (honoring the
    ...
    return f"{base}/owner/auth/callback"
...
# --------------------------------------------------------------------------- #
# routes — included by botsite/app.py, NOT under the owner gate.
# --------------------------------------------------------------------------- #
def _login_context(request: Request, configured: bool) -> dict:
    """Context for owner_login.html. botsite's base.html reads its nav/header
    context from the route (not Jinja globals), so this supplies SAFE DEFAULTS
    for every base.html variable — an empty nav, no active highlight, no add
    url, a site-ok flag (skips the feed-error banner) and empty build/counts —
    so the page renders standalone without an app-module import or KeyError."""
    return {
        "request": request,
        "configured": configured,
        "authorize_hint": AUTHORIZE_URL,
        "redirect_uri": redirect_uri_for(request),
        # base.html header/footer defaults (this is an auth-flow endpoint, not a
        # nav page — it carries no nav highlight key on purpose):
        "nav": [],
        "active": "",
        "add_url": "",
        "site_ok": True,
...
async def owner_login(request: Request):
    ...
    naming the opening owner action (add the botsite redirect URI + set the four
    vars on botsite's Railway service; SITE_PASSWORD stays an optional
    fallback). Configured → set the signed state cookie and 302 to Discord's
    authorize endpoint (scope=identify)."""
    if not oauth_configured():
        return templates.TemplateResponse(
            request, "owner_login.html", _login_context(request, configured=False)
        )
    state = make_state()
    redirect_uri = redirect_uri_for(request)
...
@router.get("/owner/auth/callback")
async def owner_auth_callback(request: Request, code: str = "", state: str = ""):
    ...
    403 (no cookie). Success redirects to botsite's owner queue (/testing/owner)."""
    if not oauth_configured():
        raise HTTPException(
            status_code=503,
            detail=(
                ...
                f"{ENV_SESSION_SECRET} on this service (SITE_PASSWORD is an "
                "optional fallback)."
            ),
        )
    ...
    if (
        ...
        raise HTTPException(
            status_code=400,
            detail="login state mismatch (CSRF check failed) — start again at /owner/login",
        )
    if not code:
    ...
    if not discord_id or discord_id != _env(ENV_OWNER_ID):
        ...
        return resp
    # botsite's owner queue lives at /testing/owner (there is no bare /owner
    # page here) — land the freshly-authorized owner straight on it.
    response = RedirectResponse("/testing/owner", status_code=302)
...
async def owner_logout(request: Request):
    """Clear the owner session and return to the public board. CSRF-protected:
    a same-origin Origin/Referer is required (a header-less or cross-origin POST
    is rejected 403), mirroring the /testing state-change floor."""
