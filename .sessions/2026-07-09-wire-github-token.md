...
> **Status:** `complete` — token set on `control-plane`, board cells verified live, docs updated; shipped as PR #3 (squash-merged into main together with the pending railway-deploy deploy content).
...
Built this PR server-side (GitHub API) from the `railway-deploy` base SHA
rather than the local branch tip: a parallel worker committed an unrelated
born-red session log (`dashboard-botsite-rework-plan`) onto the shared HEAD
mid-session. Sourcing the PR from the clean deploy SHA keeps that foreign,
still-in-flight work out of this PR. This PR lands the pending railway-deploy
deploy content (deployment.md, current-state snapshot, [D-0005]) together
with the token wiring.
