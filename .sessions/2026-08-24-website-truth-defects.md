...
> **Status:** `in-progress` — branch `codex/website-truth-defects-2026-08-24`; flips to `complete` + PR number as the deliberate LAST code step.
...
Commit and push the fully verified implementation, then flip this card complete and remove the claim as the deliberate last source step. PR #513's pinned Python 3.12 quality run will verify the ready branch. The live smoke crawl and deployed-revision re-read wait for Menno's separately approved auto-deploying merge.
...
- **Verification.** Trusted local Python 3.13.15: `python -m pytest tests/ botsite/tests dashboard/tests review/tests -q` → **2,231 passed, 5 deprecation warnings** in 3m54s. Focused adversarial regressions → **90 passed** for shared/SuperBot/Review and **33 passed** for Dashboard. `scripts/check_no_ambient_railway_ids.py` → **green**; `git diff --check` and JSON parse → **green**. `bootstrap.py check --strict` is red only on this card's designed in-progress hold and will be rerun after the final flip. The repo-local `uv` Python 3.12.14 `_ssl` Application Control wall and safe trusted-interpreter workaround are recorded in `docs/CAPABILITIES.md`; GitHub Actions supplies the pinned 3.12 verdict.
