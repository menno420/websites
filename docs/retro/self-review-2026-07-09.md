...
> **Status:** `audit` — answers to `docs/retro/QUESTIONS.md` by ID, per inbox
...
- The born-red gate leak was proven *empirically both directions*, not just
 asserted — `tests/test_born_red_session_gate.py` + the #19 incident (PR #24).
...
The `/fleet` manifest live-parse (PR #36). It maps the manager's
...
`config.FLEET_LANES` fallback (honest banner), which is safe, but nothing
...
(PR #31) tightened the session gate to require a `📊 Model:` line, but
...
borderline call was the *lane-set sync*: it was flagged ⚑ to the owner as
"keep in sync by hand or parse live," then the **next self-directed session just built
the live parse** (PR #36) under decide-and-flag. That is the correct
...
stale `complete` card masking an `in-progress` one (PR #24, regression-tested).
