...
> **Status:** `audit` — owner-directed full project self-review + wake-up pass.
...
is control-plane `/owner` (HTTP Basic on `SITE_PASSWORD`).
...
- **Auto-refresh** on the two live-monitoring surfaces (`/` + `/fleet`), 45s soft
 `#live-content` swap, pause/resume (PR #37); content pages stay static.
- **Gated `/owner` area** — un-masked board + privileged actions (cache refresh,
 re-run failed CI) (PR #14).
- Stack: FastAPI + Jinja2 + httpx, server-rendered, no build step, TTL cache.

**Kit + CI.** substrate-kit engaged (adopted PR #1; upgraded v1.0.0 → v1.2.0, PR #31). The **`quality`** workflow is a **REQUIRED** status check on `main` (owner
...
24 recorded decisions** in `docs/decisions.md` (this review appends the 24th).
**Session cards: 25** in `.sessions/` (26 with this session's).
...
| 14 | Kit-hygiene cleanup | PR #16 (8 docs rendered, quality.yml gate, ledger backfill, 7 Qs routed) ✅ | could NOT set `quality` REQUIRED — proxy blocked rulesets API (403 "Write access … not permitted through this proxy"); owner set it manually | (b) platform |
