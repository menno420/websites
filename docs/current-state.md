...
> **Status:** `living-ledger` — last updated 2026-07-09 (drop-auth session)
>
...
- **The site is built, deployed, and live** at
  ...
  **Public — no auth** ([D-0011]); the Basic-auth gate was dropped. The board's
  Actions-secrets cell is masked to a count so no secret names are exposed.
...
- **Both control-plane and dashboard are now PUBLIC** (PR #12, [D-0011]): the
  Basic-auth gate was dropped from both (owner "Yes drop the auth"); every route
  serves without credentials, `SITE_PASSWORD` is left set-but-unused. The
  control-plane board's Actions-secrets cell is masked to a **count** so no secret
  names reach the public HTML or `/api/readiness.json`. Nothing else in flight.
