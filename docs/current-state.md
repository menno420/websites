...
> **Status:** `living-ledger` — last updated 2026-07-18 (planning pass).
> The Claude Code Projects EAP **session surface** winds down ~**2026-07-21**;
> the **repo + all four Railway services stay live and writable** and the owner
> continues via normal chat afterward (no repo freeze, no deadline). The frozen
> draft backlog (#371–#380) was **cleared 2026-07-17**; the autonomous
> apparatus is being **wound down** and the Project will be **recreated**. main
> `07b4bb9` (#421); open PRs: **none** at #421; last full
> four-suite run **1928** (2026-07-18); kit v1.17.0.
>
...
trued 2026-07-17 for the fresh-start cleanup.)

- **Fresh-start cleanup in progress (2026-07-17), owner-authorized.** The 13
  frozen open PRs at recon (#359–#380) are **cleared**: the eight green
  drafts (#371/#372/#373/#376/#377/#378/#379 + the games front-door #371)
  and today's bake #380 were **merged**; the superseded bake #359 and the
  three rescue straggler cards (#361/#363/#367) were **closed**. Follow-on
  curation already landed — **#381** (arcade owner-action queue), **#382**
  (dashboard `/ideas` by lifecycle), **#383** (review evidence tally).
  **Open PRs: none.** `main` @ `ecbe2bf` (#383); strict check green.
- **The EAP session surface is winding down — the repo is NOT.** Only the
  ...
  auto-merge/sweep/bake/healthcheck workflows) is inventoried in
  `docs/NEXT-TASKS.md` → "Wind-down / retirement".
- **The four Railway services stay live and unchanged** — control-plane,
  botsite, dashboard, review all deploy from `main` (merge == deploy). The
  wind-down is about the *agent apparatus*, not the sites.
- **Owner-gated work is tracked as the 16 ⚑ asks** in
  `docs/owner/OWNER-ACTIONS.md` (mirrored in `control/status.md`). The
...
- **2026-07-18 NAV-completeness guard sweep: PRs #416→#421** (all terminal on
  `main`, HEAD `07b4bb9`): router-introspection NAV-completeness tests so every
  live route stays reachable from NAV — review #416 (`0e769d5`), then botsite +
  dashboard #418 (`13c1804`), plus #421 (`07b4bb9`) asserting off-nav pages stay
  reachable; claim cleanups #417/#419 and the coordinator-heartbeat true-up #420.
...
> lives in **`docs/NEXT-TASKS.md`** (5–8 best steps grouped by the four
> services), distilled from the two overnight veto menus
> (`docs/planning/arcade-dashboard-menu-2026-07-16.md` + the
> console/review menu) and `docs/ideas/backlog.md`. Owner console-only steps
> live in **`docs/OWNER-STEPS.md`**.
>
> **Historical:** the gen-2 queue-state NEXT list
> (`docs/planning/queue-state-2026-07-09-winddown.md`) is exhausted; the
> `control/inbox.md` order bus and the routine/seat apparatus are being
> **retired** with the wind-down (see "In flight" + `docs/NEXT-TASKS.md`).
...
3. **Clear the owner console steps** in `docs/OWNER-STEPS.md` (the
   `SITE_PASSWORD` `/owner` gate value, `BAKE_PAT` if the scheduled bake is
   kept, and dropping the two unused/unwired password variables).
