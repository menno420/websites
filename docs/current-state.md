...
> **Status:** `living-ledger` — last updated 2026-07-13 (healthcheck+truing pass: the ender truth sweep plus the same-day post-ender wave #277→#290, main `6360263`; suite 1255; bake pipeline reality)
>
...
- Quality gates: the `quality` CI workflow (`.github/workflows/quality.yml`) —
  ...
  vendored `bootstrap.py` is kit **v1.15.0** (stepwise §4.3 upgrades
  v1.6.0 → v1.15.0, PR trail #62…#199, each verified against the
...
trued 2026-07-13 at the coordinator ender.)

- **Nothing is in flight.** The 2026-07-12→13 sitting closed with zero open
  non-draft PRs and zero seams; the only open PRs are the three **draft
  kit-stub lifeboats #245/#249/#257** (auto-drafted session cards +
  `.substrate/state.json` churn preserved off-checkout — do-not-merge by
  design; landing path is an owner-click close, nothing valuable inside).
- **The remainder is owner-gated**, tracked as 8 open ⚑ six-field asks in
...
> `ANTHROPIC_API_KEY`); (2) close/dispose the parked drafts #245/#249/#257
> and run a fresh intake sweep (venture outbox markers + fm inbox).
