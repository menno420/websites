...
are now deletable, but branch deletion was 403-walled (2026-07-09) for agents
on every path (`docs/CAPABILITIES.md`), so they are left in place for the owner.
...
VERIFIED-NEEDED: attempted via the account API this session — classifier-walled (2026-07-18; verbatim "Permission for this action was denied by the Claude Code auto mode classifier. Reason: Blocked by classifier."; `docs/CAPABILITIES.md` wall entry, tried twice); requires an owner UI action. Still owner-gated. Click steps first recorded in `docs/retro/self-review-2026-07-11.md` §2.
```
