"""Module-level env-parse hardening for the dashboard (ORDER 026 finding).

On Railway an empty-string env entry is NOT "unset": before this hardening
``int(os.environ.get("DATA_CACHE_TTL_SECONDS", "180"))`` meant ``int("")`` —
...
def test_import_survives_hostile_env(monkeypatch):
    """The original crash class: reload with DATA_CACHE_TTL_SECONDS hostile."""
    for hostile in ("", "abc"):
        monkeypatch.setenv("DATA_CACHE_TTL_SECONDS", hostile)
        try:
            importlib.reload(ds)
